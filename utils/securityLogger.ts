...
export const logAuthEvent = (action: string, severity: SecurityEvent['severity'] = 'medium', details?: Record<string, any>) => {
  logSecurityEvent({
    action: `auth_${action}`,
    severity,
    details,
...
export const logSuspiciousActivity = (action: string, userId?: string, details?: Record<string, any>) => {
  logSecurityEvent({
