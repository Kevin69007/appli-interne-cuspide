...
-- Fix the sell_pet_to_shelter function to avoid the FOR UPDATE error with LEFT JOIN
...
BEGIN
  -- Start transaction
  BEGIN
    -- Lock and verify pet ownership (use separate queries to avoid LEFT JOIN + FOR UPDATE issue)
    SELECT up.*
    INTO pet_data
    ...
    SELECT p.name INTO pet_data.pet_type_name
    FROM public.pets p
