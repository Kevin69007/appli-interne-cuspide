...
-- EMERGENCY SHELTER FIX: Complete cleanup and rebuild
-- This will fix the pet transfer issue by removing all conflicting policies

-- Step 1: Drop ALL existing conflicting RLS policies on shelter_pets
...
-- Step 4: Create ONLY the essential policies needed (4 total)
-- Policy 1: Allow all inserts (SECURITY DEFINER functions need this)
CREATE POLICY "shelter_pets_insert_all" 
ON public.shelter_pets 
FOR INSERT 
WITH CHECK (true);

-- Policy 2: Allow public to view available pets
CREATE POLICY "shelter_pets_select_available" 
ON public.shelter_pets 
FOR SELECT 
USING (is_available = true);

-- Policy 3: Allow updates for adoption process
CREATE POLICY "shelter_pets_update_all" 
ON public.shelter_pets 
FOR UPDATE 
USING (true);

-- Policy 4: Allow deletes for cleanup
CREATE POLICY "shelter_pets_delete_all" 
ON public.shelter_pets 
FOR DELETE 
USING (true);

-- Step 5: Enhanced sell_pet_to_shelter function with comprehensive logging
...
BEGIN
  ...
  -- Step 5: Insert into shelter_pets (CRITICAL STEP)
  step_counter := step_counter + 1;
...
-- Step 7: Add comment for tracking
COMMENT ON TABLE public.shelter_pets IS 'Fixed shelter system with clean RLS policies - 2025-06-17';
