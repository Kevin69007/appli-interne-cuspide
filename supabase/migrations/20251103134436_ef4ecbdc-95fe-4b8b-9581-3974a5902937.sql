-- Fonction pour recalculer le score mensuel
...
-- Créer le trigger sur agenda_entries
DROP TRIGGER IF EXISTS trigger_recalculate_monthly_score ON agenda_entries;
CREATE TRIGGER trigger_recalculate_monthly_score
AFTER INSERT OR UPDATE OR DELETE ON agenda_entries
FOR EACH ROW
EXECUTE FUNCTION recalculate_monthly_score_function();

-- Fonction pour mettre à jour la cagnotte annuelle
...
-- Créer le trigger sur monthly_scores
DROP TRIGGER IF EXISTS trigger_update_annual_cagnotte ON monthly_scores;
CREATE TRIGGER trigger_update_annual_cagnotte
AFTER UPDATE OF score_global ON monthly_scores
FOR EACH ROW
WHEN (NEW.score_global IS DISTINCT FROM OLD.score_global)
EXECUTE FUNCTION update_annual_cagnotte_function();

-- Index pour optimiser les performances
CREATE INDEX IF NOT EXISTS idx_agenda_entries_employee_date ON agenda_entries(employee_id, date);
CREATE INDEX IF NOT EXISTS idx_agenda_entries_validation ON agenda_entries(statut_validation);
CREATE INDEX IF NOT EXISTS idx_monthly_scores_employee_period ON monthly_scores(employee_id, mois, annee);
CREATE INDEX IF NOT EXISTS idx_annual_cagnotte_employee_year ON annual_cagnotte(employee_id, annee);