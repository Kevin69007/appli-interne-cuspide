...
DROP TRIGGER IF EXISTS validate_pet_sale_creation_trigger ON public.pet_sales;

-- Remove the validation function since it's too aggressive
DROP FUNCTION IF EXISTS public.validate_pet_sale_on_creation();
...
-- Add a safer validation function that only validates during sale creation, not transfers
CREATE OR REPLACE FUNCTION validate_pet_sale_on_creation_only()
RETURNS TRIGGER AS $$
BEGIN
...
-- Create a safer trigger that only runs on INSERT of new sales
CREATE TRIGGER validate_pet_sale_creation_only_trigger
  BEFORE INSERT ON public.pet_sales
  FOR EACH ROW
  EXECUTE FUNCTION validate_pet_sale_on_creation_only();
