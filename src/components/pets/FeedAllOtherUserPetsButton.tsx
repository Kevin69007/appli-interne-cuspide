...
interface FeedAllOtherUserPetsButtonProps {
  targetProfile: any;
  userProfile: any;
  onComplete?: () => void;
}

const FeedAllOtherUserPetsButton = ({ 
  targetProfile, 
  userProfile,
  onComplete
}: FeedAllOtherUserPetsButtonProps) => {
  const { user } = useAuth();
  ...
  const handleFeedAll = async () => {
    ...
    // Optimistic UI: make all cards show Feeding now
    window.dispatchEvent(new CustomEvent('bulk-feed-ui-start', { detail: { targetUserId: targetProfile.id } }));

    const result = await bulkFeedPets(targetProfile.id, false, targetProfile); // isOwnPets = false
  ...
  const isFeeding = targetProfile ? isBulkFeeding(targetProfile.id) : false;
...
export default FeedAllOtherUserPetsButton;
