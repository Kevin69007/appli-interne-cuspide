...
export const ImportCategoriesDialog = ({ open, onOpenChange }: ImportCategoriesDialogProps) => {
  const { toast } = useToast();
  const queryClient = useQueryClient();
  const [file, setFile] = useState<File | null>(null);
  const [parsedData, setParsedData] = useState<CategoryRow[]>([]);
  const [isProcessing, setIsProcessing] = useState(false);

  const importCategories = useMutation({
    mutationFn: async (validRows: CategoryRow[]) => {
      const categories = validRows.map((row) => ({
        name: row.nom,
        description: row.description || null,
      }));

      const { error } = await supabase.from("product_categories").insert(categories);
      if (error) throw error;
    },
    onSuccess: () => {
      toast({ title: `✅ ${parsedData.filter((r) => r.valid).length} catégories importées` });
  ...
  const validateRow = (row: any): CategoryRow => {
    const errors: string[] = [];
  ...
  const downloadTemplate = () => {
    const template = [
      { nom: "Électricité", description: "Produits électriques et câblage" },
      { nom: "Plomberie", description: "Tuyaux et raccords" },
      { nom: "Outillage", description: "Outils manuels et électriques" },
    ];
    const ws = XLSX.utils.json_to_sheet(template);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Catégories");
    XLSX.writeFile(wb, "modele_categories.xlsx");
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Import de catégories</DialogTitle>
          <DialogDescription>
            Importez plusieurs catégories depuis un fichier CSV ou Excel
          </DialogDescription>
        </DialogHeader>
          ...
          {parsedData.length > 0 && !isProcessing && (
            <>
              ...
              <div className="flex justify-end gap-2">
                ...
                <Button
                  onClick={() => importCategories.mutate(parsedData.filter((r) => r.valid))}
                  disabled={validCount === 0 || importCategories.isPending}
                >
                  Importer {validCount} catégorie{validCount > 1 ? "s" : ""}
                </Button>
              </div>
            </>
          )}
