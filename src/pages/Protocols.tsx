...
const Protocols = () => {
  ...
  const [documents, setDocuments] = useState<Protocol[]>([]);
  const [profiles, setProfiles] = useState<Profile[]>([]);
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [isEditDialogOpen, setIsEditDialogOpen] = useState(false);
  const [editingDoc, setEditingDoc] = useState<Protocol | null>(null);
  ...
  const fetchDocuments = async () => {
    try {
      ...
      const formattedDocs = data?.map(doc => ({
        ...doc,
        tagged_profiles: doc.protocol_profiles?.map((pp: any) => pp.profiles).filter(Boolean) || []
      }));
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      toast.success("Protocole supprimé avec succès");
      fetchDocuments();
    } catch (error) {
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error: deleteError } = await supabase
        .from("protocol_profiles")
        .delete()
        .eq("protocol_id", editingDoc.id);

      if (deleteError) throw deleteError;

      const profileLinks = formData.selectedProfiles.map(profileId => ({
        protocol_id: editingDoc.id,
        profile_id: profileId,
      }));
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-bold">Protocoles</h1>
              <p className="text-muted-foreground">
                Gérez les protocoles et leurs accès
              </p>
            </div>
          </div>
          ...
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            ...
            <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
              <DialogHeader>
                <DialogTitle>Créer un protocole</DialogTitle>
                <DialogDescription>
                ...
                <div className="space-y-4 py-4">
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      placeholder="Ex: Protocole de coulée"
                    />
                  </div>
        ...
        <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
          <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
            ...
            <form onSubmit={handleUpdate}>
              <div className="space-y-4 py-4">
                <div className="space-y-2">
                  <Label htmlFor="edit-name">Nom du protocole *</Label>
                  <Input
                    id="edit-name"
                    value={formData.name}
                    onChange={(e) =>
